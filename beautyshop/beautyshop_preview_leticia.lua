...
function PREVIEW_LETICIA_SHOP_OPEN()
    local topFrame = ui.GetFrame("beautyshop");
	if topFrame == nil then
		return
	end
	
	local shopName = ClMsg("PREVIEW_LETICIA");
...
function PREVIEW_LETICIA_SHOP_INIT_FUNCTIONMAP()
	beautyShopInfo.functionMap["UPDATE_SUB_ITEMLIST"] = nil
	beautyShopInfo.functionMap["DRAW_ITEM_DETAIL"] = PREVIEW_LETICIA_SHOP_DRAW_ITEM_DETAIL
	beautyShopInfo.functionMap["POST_SELECT_ITEM"] = PREVIEW_LETICIA_SHOP_POST_SELECT_ITEM
...
function PREVIEW_LETICIA_SHOP_GET_SHOP_ITEM_LIST()
	PREVIEW_LETICIA_SHOP_MAKE_ITEMLIST();
end

function PREVIEW_LETICIA_SHOP_REGISTER_ITEM_LIST()
	if PreviewItemList == nil then
		PreviewItemList={}
		
		local clsList, cnt = GetClassList('Beauty_Shop_Preview_Leticia');
		for i = 0, cnt - 1 do
			...
			local data = {
                ...
                IDSpace = 'Beauty_Shop_Preview_Leticia',
 ...
 function PREVIEW_LETICIA_SHOP_MAKE_ITEMLIST(gender)
	local list = PREVIEW_LETICIA_SHOP_REGISTER_ITEM_LIST();
	BEAUTYSHOP_UPDATE_ITEM_LIST(list, #list);
end

function PREVIEW_LETICIA_SHOP_GET_ITEM_EQUIPTYPE(ItemClassName)
	local list = PREVIEW_LETICIA_SHOP_REGISTER_ITEM_LIST();
    for i = 1, #list do
        local data = list[i];    
...
function PREVIEW_LETICIA_SHOP_POST_SELECT_ITEM(frame, ctrl)
	...
	local equipType = PREVIEW_LETICIA_SHOP_GET_ITEM_EQUIPTYPE(itemClassName);
...
function PREVIEW_LETICIA_SHOP_DRAW_ITEM_DETAIL(obj, itemobj, ctrlset)
