	...
	for i = 0, MAX_SLOT_CNT - 1 do
		...
		if true == updateslot and quickSlotInfo.category ~= 'NONE' then
			local slot = GET_CHILD_RECURSIVELY(frame, "slot"..i+1, "ui::CSlot");
				...
				local change_list = GET_QUICKSLOT_CHANGE_SKILL_LIST();
				if quickSlotInfo.category == 'Skill' then
					if change_list[tostring(quickSlotInfo.type)] == nil then
						...
						local change_skill = true;
						local to_type = change_list[tostring(quickSlotInfo.type)];
						SET_QUICK_SLOT(nil, slot, 'Skill', to_type, 0, 0, false, true, false, change_skill);
					end
				end
		    end  
