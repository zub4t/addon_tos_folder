function TRADESELECTITEM_2_ON_INIT(addon, frame)
end

function OPEN_SKILL_RESET_SELECT_JOB(invItem)
	...
	local frame = ui.GetFrame("tradeselectitem_2")
	...
	useBtn:SetEventScript(ui.LBUTTONUP,'REQUEST_SKILL_RESET_JOB')	
...
function REQUEST_SKILL_RESET_JOB(frame, ctrl, argStr, argNum)    
	local box = frame:GetChild('box');
	tolua.cast(box, "ui::CGroupBox");
	...
	if selectExist == 1 then
        ...
        pc.ReqExecuteTx_Item("SCR_USE_SKILL_RESET_JOB", itemGuid, argStr);		
	end
