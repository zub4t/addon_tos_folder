...
--------------------- PHARMACY 시작 ---------------------
function PHARMACY_EXTRACT_FULLDARK_UI_OPEN(itemClassName, itemCount)
    local frame = ui.GetFrame("fulldark_checkbtn")
	frame:EnableHide(0)

    local imageName = "alchemy_item_show"
	local posX = ui.GetSceneWidth() / 2
    local posY = ui.GetSceneHeight() / 2
    movie.PlayUIEffect(frame:GetUserConfig("GACHA_EFT_2"), posX, posY, tonumber(frame:GetUserConfig("GACHA_EFT_SCALE_2")))

    imcSound.PlaySoundEvent('sys_pharmacy_grinder_reward')

    local itembgimg = GET_CHILD(frame, "itembgimg")
    itembgimg:Resize(515, 576)
    itembgimg:SetImage(imageName)
    ...
    local checkBtn = GET_CHILD(frame, "checkBtn")
	checkBtn:SetEventScript(ui.LBUTTONUP, "PHARMACY_EXTRACT_FULLDARK_UI_CLOSE")

    frame:ShowWindow(1)
end

function PHARMACY_EXTRACT_FULLDARK_UI_CLOSE()
    ui.FlushGachaDelayPacket()
	PHARMACY_UI_EXTRACTOR_COMPLETE()
