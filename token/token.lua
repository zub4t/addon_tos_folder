...
function BEFORE_APPLIED_TOKEN_OPEN(invItem)
	...
	local ctrlSet = gBox:CreateControlSet("tokenDetail", "CTRLSET_" .. 12,  ui.CENTER_HORZ, ui.TOP, 0, 0, 0, 0);
	local prop = ctrlSet:GetChild("prop");
	local imag = string.format("{img 2multiply_image %d %d}", 55, 45)
	prop:SetTextByKey("value", imag..ClMsg("TOSCoin_Increase_Holdings"));
	local value = GET_CHILD_RECURSIVELY(ctrlSet, "value");
	value:ShowWindow(0);
