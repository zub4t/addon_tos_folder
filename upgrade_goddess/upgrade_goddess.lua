...
function UPGRADE_GODDESS_ON_INIT(addon, frame)
	addon:RegisterMsg("OPEN_DLG_UPGRADE_GODDESS", "ON_OPEN_DLG_UPGRADE_GODDESS");
	addon:RegisterMsg("UPGRADE_EVIL_GODDESS_SUCCESS", "UPGRADE_EVIL_GODDESS_SUCCESS");
	addon:RegisterMsg("UPGRADE_EVIL_GODDESS_FAIL", "UPGRADE_EVIL_GODDESS_FAIL");
end
...
function release_ui_lock_goddess()
	local frame = ui.GetFrame("upgrade_goddess");
...
local function PLAY_EXEC_EFFECT_GODDESS()
	local frame = ui.GetFrame("upgrade_goddess");
	local EXTRACT_RESULT_EFFECT_NAME = frame:GetUserConfig('EXTRACT_RESULT_EFFECT');
	...
	ui.SetHoldUI(true);	
    ReserveScript('release_ui_lock_goddess()', EFFECT_DURATION);
end

function release_ui_effect_for_goddess()
	local frame = ui.GetFrame("upgrade_goddess");
...
function ON_OPEN_DLG_UPGRADE_GODDESS()
	ui.OpenFrame("upgrade_goddess");
end

function UPGRADE_GODDESS_OPEN(frame)
    UPGRADE_GODDESS_UI_RESET();
	
	INVENTORY_SET_CUSTOM_RBTNDOWN("UPGRADE_GODDESS_INV_RBTNDOWN");
	ui.OpenFrame("inventory");
end

function UPGRADE_GODDESS_CLOSE(frame)
...
function UPGRADE_GODDESS_SLOT_RESET(slot, slotIndex)	
	if IS_CHECK_UPGRADE_GODDESS_RESULT() == true then
		return;
	end
	...
	local gauge = GET_CHILD_RECURSIVELY(ui.GetFrame("upgrade_goddess"), "refine_gauge_" .. slotIndex);		
	gauge:SetPoint(0, 100)
	gauge:ShowWindow(0);	

	if slotIndex > 1 then		
		local text = GET_CHILD_RECURSIVELY(ui.GetFrame("upgrade_goddess"), "text_warning");
		text:ShowWindow(0)
	end
end

function UPGRADE_GODDESS_UI_RESET()
	local frame = ui.GetFrame("upgrade_goddess");
    ...
    local slot_count = GET_UPGRADE_VIROBA_SOURCE_COUNT();  -- 바이보라랑 같이 사용
	for i = 1, slot_count do 
		local slot = GET_CHILD_RECURSIVELY(frame, "slot_"..i);
		UPGRADE_GODDESS_SLOT_RESET(slot, i);
	end
	...
	UPGRADE_GODDESS_MATERIAL_UI_RESET();
end

function UPGRADE_GODDESS_MATERIAL_UI_RESET()			
	if IS_CHECK_UPGRADE_GODDESS_RESULT() == true then
		return;
	end
...
function UPGRADE_GODDESS_SAME_ITEM_CHECK(guid, classname)
	local frame = ui.GetFrame("upgrade_goddess");
	for i = 1, 2 do 
		local slot = GET_CHILD_RECURSIVELY(frame, "slot_"..i);
...
function UPGRADE_GODDESS_ITEM_REG(guid, ctrl, slotIndex)			
	if ui.CheckHoldedUI() == true then
		return;
    end
	...
	local ret, ret_classname, cur_lv = CAN_UPGRADE_GODDESS(itemObj);		
	if ret == false or ret_classname == 'None' or cur_lv == 0 then
		ui.SysMsg(ClMsg('WrongDropItem'))
        return;
	end
	
	local result_item_classname = GET_UPGRADE_GODDESS_ITEM_NAME(itemObj);
	local cls = GetClass('Item', result_item_classname)
	if cls == nil then
		return;
	end

	if UPGRADE_GODDESS_SAME_ITEM_CHECK(guid, ret_classname) == false then
	...
	local refine_count = TryGetProp(itemObj, 'UPGRADE_GODDESS_TRY_COUNT', 0)
	local gauge = GET_CHILD_RECURSIVELY(ui.GetFrame("upgrade_goddess"), "refine_gauge_" .. slotIndex)
	gauge:SetPoint(refine_count, GET_UPGRADE_GODDESS_MAX_COUNT(cur_lv + 1))
	gauge:ShowWindow(1);
	...
	UPGRADE_GODDESS_RESULT_ITEM_INIT(result_item_classname);
	UPGRADE_GODDESS_MATERIAL_INIT();
end

function UPGRADE_GODDESS_ITEM_DROP(parent, ctrl, argStr, slotIndex)
	local liftIcon = ui.GetLiftIcon();
	local FromFrame = liftIcon:GetTopParentFrame();
	if FromFrame:GetName() == 'inventory' then
		local iconInfo = liftIcon:GetInfo();
		UPGRADE_GODDESS_ITEM_REG(iconInfo:GetIESID(), ctrl, slotIndex);
	end
end

function UPGRADE_GODDESS_ITEM_POP(parent, ctrl, argStr, slotIndex)	
	if ui.CheckHoldedUI() == true then
		return;
	end

	UPGRADE_GODDESS_SLOT_RESET(ctrl, slotIndex);
	UPGRADE_GODDESS_MATERIAL_UI_RESET();
...
function UPGRADE_GODDESS_RESULT_ITEM_INIT(classname)
	local frame = ui.GetFrame("upgrade_goddess");
...
function UPGRADE_GODDESS_MATERIAL_INIT()
	local frame = ui.GetFrame("upgrade_goddess");
	...
	local silver_cost = GET_UPGRADE_GODDESS_SILVER_COST(goal_lv)
...
function UPGRADE_GODDESS_MATERIAL_REG(guid)	
	if ui.CheckHoldedUI() == true then
		return;
    end
	...
	local dic_misc, misc_size, dic_index = GET_UPGRADE_GODDESS_MISC_LIST(goal_lv)		
	...
	if IS_UPGARDE_GODDESS_MISC(material_class_name, dic_misc) == false then
		return;
	end
...
function UPGRADE_GODDESS_MATERIAL_DROP(parent, ctrl)
	...
	if FromFrame:GetName() == 'inventory' then
		local iconInfo = liftIcon:GetInfo();
		UPGRADE_GODDESS_MATERIAL_REG(iconInfo:GetIESID());
	end
end

function UPGRADE_GODDESS_MATERIAL_POP(parent, ctrl)
	UPGRADE_GODDESS_MATERIAL_UI_RESET();
end

function UPGRADE_GODDESS_INV_RBTNDOWN(itemObj, slot)
	if ui.CheckHoldedUI() == true then
		return;
	end
	...
	for i = 1, slot_count do 
		...
		if ctrl:GetIcon() == nil then
			UPGRADE_GODDESS_ITEM_REG(iconInfo:GetIESID(), ctrl, i);
			return;
		end
	end
...
function UPGRADE_GODDESS_BTN_CLICK(parent, ctrl)
	if ui.CheckHoldedUI() == true then
		return;
	end
...
function UPGRADE_EVIL_GODDESS_SUCCESS(frame, msg, guid)	
	local frame = ui.GetFrame("upgrade_goddess");
